...
func makeKernel() drawable.Drawable {
	hstack := stack.NewHStackDrawable()

	ascii := line.LineDrawableFromLines(
		*text.NewLine(`  .·:·.                `),
		*text.NewLine(`  :   :          /\    `),
		*text.NewLine(`  '·.·'    /\   /  \   `),
		*text.NewLine(`          /  \ /    \  `),
	...
	box := box.BoxDrawableFromDrawable(ascii)
	pst := position.NewPositionDrawable(box).
		PositionY(style.Top).
		MarginY(1).MarginX(1).Absolute(false)

	pst2 := position.NewPositionDrawable(pst.ToDrawable()).
		PositionY(style.Top).
		MarginY(3)
